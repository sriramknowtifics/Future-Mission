            ...
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                ...
                <div>
                    <select class="form-select form-select-sm"
                        ...
                        <option value="{{ route('shop.index', array_merge(request()->all(),['sort'=>'price_asc'])) }}"
                            @selected(request('sort')=='price_asc')>
                            Price: Low to High
                        </option>

                        <option value="{{ route('shop.index', array_merge(request()->all(),['sort'=>'price_desc'])) }}"
                            @selected(request('sort')=='price_desc')>
                            Price: High to Low
                        </option>

                        <option value="{{ route('shop.index', array_merge(request()->all(),['sort'=>'newest'])) }}"
