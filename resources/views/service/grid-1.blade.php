...
@section('content')
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="navigator-breadcrumb-wrapper">
                        ...
                        @if($current != NULL)
                        <a id="breadcrumb-current" class="current" href="{{route('service.index')}}">{{ $current}}</a>
                        @else
                        <a id="breadcrumb-current" class="current" href="{{route('service.index')}}">All Services</a>
  ...
  <div class="shop-grid-sidebar-area rts-section-gap">
    ...
    <div class="single-filter-box">
        ...
        <form action="{{ route('service.index') }}" method="GET" class="filterbox-body">
              ...
              <div class="filterbox-body">
                <ul class="list-unstyled">
                  @foreach ($categories as $cat)
                     <li >
                    <a href="{{ route('service.index', ['category' => $cat->id]) }}" class="menu-item">
            ...
            <div class="single-filter-box mt-3">
              ...
              <div class="filterbox-body">
                ...
                <ul class="list-unstyled">
                  @foreach ($categories as $cat)
                    <li><a href="{{ route('service.index', ['category' => $cat->id]) }}">Brands</a></li>
                  @endforeach
                </ul>
              </div>
            </div>
            ...
            <div class="single-filter-box mt-3">
              ...
              <div class="filterbox-body">
                <ul class="list-unstyled">
                    <li><a href="{{ route('service.index', ['category' => 1]) }}">In Stock</a></li>
                    <li><a href="{{ route('service.index', ['category' => 2]) }}">On Sale</a></li>
            ...
            <div class="col-xl-12">
                <div class="row">
                    <div class="col-12 mb-4">
                        <div class="title-area-between">
                            <h3 class="title-left">All Services</h3>
                            <div class="sorting-area">
                                <select class="form-select" onchange="window.location.href=this.value">
                                    <option value="{{ route('service.index', request()->except('sort')) }}">Default Sorting</option>
                                    <option value="{{ route('service.index', array_merge(request()->all(), ['sort' => 'price_asc'])) }}" {{ request('sort') == 'price_asc' ? 'selected' : '' }}>Price: Low to High</option>
                                    <option value="{{ route('service.index', array_merge(request()->all(), ['sort' => 'price_desc'])) }}" {{ request('sort') == 'price_desc' ? 'selected' : '' }}>Price: High to Low</option>
                                    <option value="{{ route('service.index', array_merge(request()->all(), ['sort' => 'newest'])) }}" {{ request('sort') == 'newest' ? 'selected' : '' }}>Newest First</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row g-4">
                    @forelse($services as $service)
                        @php
                            $img = $service->images->first();
                            $oldPrice = $service->price * 1.4;
                            $discount = 25;
                        @endphp
                        <div class="col-xxl-3 col-xl-4 col-lg-4 col-md-6 col-sm-6">
                            <div class="single-shopping-card-one">
                                <div class="image-and-action-area-wrapper">
                                    <a href="{{ route('products.show', $service->id) }}" class="thumbnail-preview">
                                        @if($discount)
                                            <div class="badge">
                                    ...
                                    <div class="action-share-option">
                                        <span class="single-action openuptip wishlist-btn" data-id="{{ $service->id }}" title="Add to Wishlist">
                                            <i class="fa-light fa-heart"></i>
                                        </span>
                                        <span class="single-action openuptip" title="Compare">
                                            <i class="fa-solid fa-arrows-retweet"></i>
                                        </span>
                                        <span class="single-action openuptip quickview-btn" data-id="{{ $service->id }}" title="Quick View">
                                ...
                                <div class="body-content text-center">
                                    <a href="{{ route('products.show', $service->id) }}">
                                        <h4 class="title">{{ \Illuminate\Support\Str::limit($service->name, 50) }}</h4>
                                    </a>
                                    <span class="availability text-muted">{{ optional($service->category)->name ?? 'Uncategorized' }}</span>

                                    <div class="price-area my-3">
                                        <span class="current text-danger fw-bold">₹{{ number_format($service->price, 2) }}</span>
                                        @if($discount)
                    ...
                    @empty
                        <div class="col-12 text-center py-5">
                            ...
                            <a href="{{ route('service.index') }}" class="btn btn-primary mt-3">Browse All Products</a>
                        </div>
                    @endforelse
                </div>
